...
@Component({
  ...
  providers: [provideNativeDateAdapter(), { provide: MAT_DATE_LOCALE, useValue: 'ru' }],
  imports: [Grid, GridRow, GridCell, GridCellWidget],
})
export class Calendar<D = Date> {
  ...
  private readonly _today = this._dateAdapter.today();
  ...
  readonly weekdays: Signal<{ long: string; narrow: string }[]> = computed(() => {
    ...
    const weekdays = longWeekdays.map((long: string, i: number) => {
  ...
  readonly calendar = computed(() => {
    ...
    for (let i = 0, cell = this._firstWeekOffset(); i < daysInMonth; i++, cell++) {
      ...
      const isPast = this._dateAdapter.compareDate(date, this._today) < 0;
